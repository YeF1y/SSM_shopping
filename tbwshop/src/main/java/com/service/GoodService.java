...
public class GoodService {
	...
	public List<Goods> getListByUserId(int userId, int status, int page, int size){
		if (status == 0) {
			return packTopList(goodDao.getListByUserId(userId, size * (page-1), size));
		}
		return null;
	}
	...
	public long getTotalByUserId(int userId, int status){
		if (status == 0) {
			return goodDao.getTotalByUserId(userId);
		}
		return 0;
	}
	/**
	 ...
	 * @return
	 */
	public List<Goods> getAllList( int page, int size){
		return goodDao.getAllList(size * (page-1), size);
	}
}