...
public interface GoodsDao {
    ...
    @Select("select * from goods where adminId=#{userId} order by id desc limit #{begin}, #{size}")
	public List<Goods> getListByUserId(@Param("userId")int userId, @Param("begin")int begin, @Param("size")int size);
	/**
	 * 获取总数
	 * @return
	 */
    ...
    @Select("select count(*) from goods where  adminId=#{userId}")
	public long getTotalByUserId(@Param("userId")int userId);
	/**
	 * 通过类型获取列表
	 * @param typeid
	 * @param begin
	 ...
	 */
    @Select("select * from goods order by id desc limit #{begin}, #{size}")
	public List<Goods> getAllList(@Param("begin")int begin, @Param("size")int size);
